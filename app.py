...
def poisson(lmbda, k):
    """Cálculo de probabilidad de goles."""
    if lmbda <= 0: return 1.0 if k == 0 else 0.0
    return (math.exp(-lmbda) * (lmbda**k)) / math.factorial(k)

def get_matches():
    """Obtiene el historial de 318 partidos para el motor matemático."""
    matches = []
...
def predict():
    ...
    def stats(name):
        ...
        # Si el equipo es nuevo (recién ascendido), usamos fuerza promedio 1.0
...
@app.route('/api/teams')
def teams():
    """Filtra y devuelve solo los 16 equipos de la temporada actual."""
    try:
        # Obtenemos el calendario de la temporada 2025 para ver quién juega hoy
        url = "https://v3.football.api-sports.io/fixtures"
        params = {"league": LIGA_I_ID, "season": 2025}
        resp = requests.get(url, headers=HEADERS, params=params).json()
        
        # Extraemos equipos que tienen partidos programados este año
        lista_actual = []
        for item in resp.get('response', []):
            lista_actual.append(item['teams']['home']['name'])
        
        t_list = sorted(list(set(lista_actual)))
        
        # Si la API no devuelve exactamente 16, usamos la lista oficial de respaldo
        if len(t_list) < 14: 
            raise Exception("Lista incompleta")
            
        return jsonify({'teams': [{'team': x} for x in t_list]})
    
    except Exception as e:
        # Lista Manual de los 16 de la Liga I (Temporada 25/26)
        backup = [
        ...
        return jsonify({'teams': [{'team': x} for x in sorted(backup)]})
