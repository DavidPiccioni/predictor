...
# LISTA 100% REAL DE PRIMERA DIVISIÓN (LIGA I 25/26)
EQUIPOS_OFICIALES = [
...
def get_matches():
    ...
    for year in [2024, 2025]:
        ...
        # Si no hay API_KEY, esto fallará y predict() sabrá que no puede calcular
...
def predict():
    try:
        ...
        if not matches:
            return jsonify({"error": "No se pudieron obtener datos. Revisa la API Key en Render."}), 500
...
@app.route('/api/teams')
def teams():
    return jsonify({'teams': [{'team': x} for x in sorted(EQUIPOS_OFICIALES)]})
