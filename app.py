...
def poisson(lmbda, k):
    """Cálculo matemático de probabilidad de goles."""
    if lmbda <= 0: return 1.0 if k == 0 else 0.0
    return (math.exp(-lmbda) * (lmbda**k)) / math.factorial(k)

def get_matches():
    """Obtiene el historial de partidos finalizados (FT) de 2024 y 2025."""
    matches = []
...
def predict():
    ...
    # Promedio de goles de la liga para normalizar fuerzas
    avg_l = sum(m['gh'] + m['ga'] for m in matches) / (len(matches) * 2)

    def stats(name):
        t_m = [m for m in matches if m['h'] == name or m['a'] == name]
        # PROTECCIÓN: Si el equipo es nuevo y no tiene partidos, asignamos fuerza neutra (1.0)
        if not t_m:
...
@app.route('/api/teams')
def teams():
    """Obtiene la lista oficial de los 16 equipos de la temporada 2025."""
    try:
        url = "https://v3.football.api-sports.io/teams"
        params = {"league": LIGA_I_ID, "season": 2025}
        resp = requests.get(url, headers=HEADERS, params=params).json()
        
        # Extraemos los nombres de los equipos inscritos oficialmente
        t_list = sorted([item['team']['name'] for item in resp.get('response', [])])
        
        if not t_list: raise Exception("Lista vacía")
        return jsonify({'teams': [{'team': x} for x in t_list]})
    
    except Exception as e:
        # Fallback: Si la API falla, usa los equipos encontrados en los partidos históricos
        m = get_matches()
